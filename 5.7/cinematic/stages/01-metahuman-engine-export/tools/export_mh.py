...
def _groom_texture2d_deps(groom_asset):
    ...
    *_CardsAtlas_Tangent, plus strand-space textures used by the Niagara hair system
    ...
    for d in deps or []:
        dep_pkg = str(d)
        # 5.7 Optimized moves groom card atlases into the MetaHumanCharacter
        # plugin's shared content folder (mount point /MetaHumanCharacter/)
        # instead of copying per-character. Accept both /Game/ (5.6 behaviour
        # + per-char exports still there) and /MetaHumanCharacter/ (plugin).
        if not (dep_pkg.startswith("/Game/")
                or dep_pkg.startswith("/MetaHumanCharacter/")):
            continue
        # Load by conventional <pkg>.<leaf> name first — works for both
        # /Game/ and /MetaHumanCharacter/ plugin mount. Fall back to the
        # legacy per-asset iteration only if that lookup misses.
        leaf = dep_pkg.rsplit("/", 1)[-1]
        t = None
        try:
            t = unreal.EditorAssetLibrary.load_asset(f"{dep_pkg}.{leaf}")
        except Exception as e:
            _log(f"  groom dep load failed {dep_pkg}: {e}")
            t = None
        _log(f"  groom dep {dep_pkg} -> {type(t).__name__ if t else 'None'}")
        if isinstance(t, unreal.Texture2D):
            # Plugin-mount textures can be loaded but AssetExportTask
            # refuses to dump them (read-only plugin content). Duplicate
            # into the character's Grooms/Textures/ so the standard export
            # path succeeds. Per-project /Game/ assets stay as-is.
            if dep_pkg.startswith("/MetaHumanCharacter/"):
                groom_pkg = str(groom_asset.get_path_name()).split(".")[0]
                groom_folder = groom_pkg.rsplit("/", 1)[0]
                dst_folder = f"{groom_folder}/Textures"
                dst_path = f"{dst_folder}/{leaf}"
                if not unreal.EditorAssetLibrary.does_asset_exist(dst_path):
                    try:
                        unreal.EditorAssetLibrary.duplicate_asset(dep_pkg, dst_path)
                    except Exception as e:
                        pass
                try:
                    dup = unreal.EditorAssetLibrary.load_asset(f"{dst_path}.{leaf}")
                except Exception:
                    dup = None
                if isinstance(dup, unreal.Texture2D):
                    t = dup
            out.append(t)
            continue
...
def _lookup_hair_card_material_for(mh_folder, comp_name):
    ...
    # 5.6 priorities (exact MI names) first; if that misses (5.7 projects
    # don't ship those exact names under /Game/<Name>/), fall back to the
    # 5.7 naming convention `MI_WI_<Groom>_Hair_Cards` / `_Facial_Hair` /
    # `_Hair`. Derive <Groom> from the component name (e.g.
    # mustache_m_curly_cardsmesh_group0_lod0 -> mustache_m_curly).
    groom_stem = comp_name
    idx = groom_stem.find("_cardsmesh")
    if idx != -1:
        groom_stem = groom_stem[:idx]
    if want_facial:
        priorities_57 = [
            f"mi_wi_{groom_stem}_facial_hair",
            f"mi_wi_{groom_stem}_hair_cards",
            f"mi_wi_{groom_stem}_hair",
        ]
    else:
        priorities_57 = [
            f"mi_wi_{groom_stem}_hair_cards",
    ...
    for name_key in priorities + priorities_57:
...
def _iter_textures_in_material(material_interface):
    ...
    try:
        ...
        for dep in deps:
            dep_str = str(dep)
            # Same 5.7 plugin-path allowance as the groom walker: the MH
            # plugin mounts /MetaHumanCharacter/ and hosts shared assets
            # (hair atlases, shared face functions) there.
            if not (dep_str.startswith("/Game/")
                    or dep_str.startswith("/MetaHumanCharacter/")):
                continue
...
def main(char=None, workspace=None):
    ...
    # 5.7: the MetaHumanCharacter plugin ships groom card atlases
    # (CardsAtlas_Attribute / Tangent / Coverage, ColorXYDepthGroupID) as
    # *Optional* content. AssetRegistry sees them as dependencies when
    # walking /Game/<Name>/Grooms/<Groom> but load_asset returns None
    # unless we explicitly force a synchronous AR scan of the Optional
    # roots. Do that once up-front so the downstream groom dep walker
    # resolves every referenced plugin texture to a real Texture2D.
    try:
        _asset_registry().scan_paths_synchronous([
            "/MetaHumanCharacter/Optional/Grooms/GroomAssets",
            # 5.7 Optimized eye MIs only ship per-char T_EyeIris{L,R}_BC
            # baked photos. The 5.6 procedural iris shader that stage 02
            # uses needs the 5.6 packed atlas (T_Iris_A_M etc) with R=
            # fibril, B=limbus. Epic still ships those atlases in the
            # plugin — mount so we can export + prefer them for eye MIs.
            "/MetaHumanCharacter/Lookdev_UHM/Eye/Textures/Iris",
        ], force_rescan=True)
        _log("mounted plugin groom + iris texture folders")
    except Exception as e:
        _log(f"WARN scan_paths_synchronous failed: {e}")
    ...
    for mesh in hair_card_meshes:
        ...
        # 5.7: hair-card override MI is not always present under /Game/<Name>/
        # (it's a MI_WI_<Groom>_Hair_Cards which the 5.6 helper doesn't match).
        # We still want the groom's atlas textures regardless, so don't gate
        # on override_mi. If it's None, we associate the atlas with the MI
        # the StaticMesh carries natively (often WorldGridMaterial — stage 02
        # swaps it for the right one by comp_name anyway).
        if groom is not None:
            override_mi_path = override_mi.get_path_name() if override_mi is not None else ""
    ...
    # --- Strand-only grooms (no cards mesh): export their atlases anyway --- #
    # Eyelashes_S_Sparse / Beard_M_Stubble ship strand-only (no
    # _CardsMesh_Group0_LOD0 StaticMesh). Their coverage/tangent atlases
    # live on the Groom asset itself (often plugin-mount under
    # /MetaHumanCharacter/Optional/Grooms/GroomAssets/...). We still want
    # them exported + associated with the face's eyelash/fuzz slot MI
    # (`MI_Face_EyelashesHiLODs` etc) so stage 02 can wire alpha.
    try:
        grooms_folder = f"{mh_folder}/Grooms"
        ar = _asset_registry()
        groom_filt = unreal.ARFilter(
            package_paths=[grooms_folder],
            class_paths=[unreal.TopLevelAssetPath("/Script/HairStrandsCore", "GroomAsset")],
            recursive_paths=True,
        )
        groom_data = ar.get_assets(groom_filt) or []
    except Exception:
        groom_data = []
    # Map of keyword → face mesh MI path that owns the slot; fallback per
    # character: we look up the MI by name in the manifest's face-mesh
    # material records (built above in mesh_records for the face).
    face_mesh_rec = next((r for r in mesh_records
                          if r.get("role") == "face"), None)
    face_mat_paths = [m["material"] for m in (face_mesh_rec.get("materials", []) if face_mesh_rec else [])]
    _log(f"strand-groom walker: {len(groom_data)} grooms, face MIs: {[p.rsplit('.',1)[-1] for p in face_mat_paths]}")
    def _find_face_mi(keyword):
        kw = keyword.lower()
        for mp in face_mat_paths:
            if kw in mp.lower():
                return mp
        return None
    for gd in groom_data:
        try:
            g = gd.get_asset()
        except Exception:
            g = None
        if g is None:
            continue
        gn = g.get_name().lower()
        # Route by groom name keyword → face slot kind's MI on the face mesh.
        target_mi = None
        hint_comp = gn
        if "eyelash" in gn:
            target_mi = _find_face_mi("eyelashes") or _find_face_mi("lash")
        elif "stubble" in gn or "peachfuzz" in gn or "beard" in gn:
            target_mi = _find_face_mi("fuzz") or _find_face_mi("facialhair") or _find_face_mi("face_hair")
        if not target_mi:
            _log(f"  strand-groom {gn}: no matching face MI (routing map miss)")
            continue
        _log(f"  strand-groom {gn} -> MI {target_mi.rsplit('.',1)[-1]}")
        groom_deps = _groom_texture2d_deps(g)
        if "eyelash" in gn and not groom_deps:
            # Eyelash grooms in 5.7 reference plugin MIs
            # (MI_Eyelashes_HigherLODs) that are 2 hops past our walker.
            # Fall back to the 5.6 Ada coverage atlas shipped under
            # references/fallbacks/.
            fallback = os.path.normpath(os.path.join(
                os.path.dirname(os.path.abspath(__file__)),
                "..", "references", "fallbacks",
                "eyelashes_fallback_coverage.png",
            ))
            if os.path.exists(fallback):
                import shutil
                dest = os.path.join(paths["out_root"], "textures",
                                    "Eyelashes_L_SlightCurl_Coverage.tga")
                try:
                    shutil.copyfile(fallback, dest)
                    _log(f"  strand-groom {gn}: 5.6 fallback coverage copied")
                    # No component_hint here: stage 02's hint filter is
                    # keyed on mesh component (face here), and setting a
                    # groom-scoped hint would cause the wiring step to
                    # skip this record.
                    texture_records.append({
                        "asset_path": "Eyelashes_Fallback_Coverage",
                        "file_path": "textures/Eyelashes_L_SlightCurl_Coverage.tga",
                        "material": target_mi,
                        "param": None,
                        "source": "eyelash_fallback",
                    })
                except Exception as e:
                    _log(f"  strand-groom {gn}: fallback copy failed: {e}")
            continue
        for tex in groom_deps:
            tp = tex.get_path_name()
            tex_rel = f"textures/{tex.get_name()}.tga"
            tex_abs = os.path.join(paths["out_root"], tex_rel)
            ...
            try:
                ...
                warnings.append(f"groom-only texture export failed: {tp}: {e}")
                continue
            texture_records.append({
                "asset_path": tp,
                "file_path": tex_rel,
                ...
                "param": None,
                "source": "groom_atlas",
                "component_hint": hint_comp,
            })

    # --- 5.7 eye-shader compatibility: ship the 5.6 iris atlas --------- #
    # The 5.6 procedural iris shader (preserved in stage 02) samples R=
    # fibril detail and B=limbus from a packed atlas (T_Iris_A_M.tga).
    # 5.7 Optimized per-char iris textures (T_EyeIris{L,R}_BC) are baked
    # photographs with no fibril/limbus channels — the shader output on
    # them is visually wrong. Export the plugin-shipped 5.6-style atlas
    # and associate it with the face mesh's eye-refractive MIs so the
    # stage-02 basecolor picker prefers it.
    iris_atlas_pkg = "/MetaHumanCharacter/Lookdev_UHM/Eye/Textures/Iris/T_Iris_A_M"
    try:
        iris_atlas = unreal.EditorAssetLibrary.load_asset(
            f"{iris_atlas_pkg}.T_Iris_A_M")
    except Exception:
        iris_atlas = None
    if isinstance(iris_atlas, unreal.Texture2D):
        tex_rel = "textures/T_Iris_A_M.tga"
        tex_abs = os.path.join(paths["out_root"], tex_rel)
        try:
            _export_texture(iris_atlas, tex_abs)
            # Target every face-mesh MI that stage 02 classifies as
            # eye_refractive — i.e. names like MI_EyeL_Baked / MI_EyeR_Baked.
            if face_mesh_rec:
                for mat_rec in face_mesh_rec.get("materials", []):
                    mi_path = mat_rec["material"]
                    leaf = mi_path.rsplit(".", 1)[-1].lower()
                    if "eyel_baked" in leaf or "eyer_baked" in leaf:
                        texture_records.append({
                            "asset_path": iris_atlas.get_path_name(),
                            "file_path": tex_rel,
                            "material": mi_path,
                            "param": None,
                            "source": "iris_atlas_5_6_compat",
                        })
            _log("exported 5.6-compat T_Iris_A_M.tga for eye MIs")
        except Exception as e:
            _log(f"WARN T_Iris_A_M export failed: {e}")
    else:
        _log("WARN T_Iris_A_M not loadable — 5.7 eyes will use photo texture")

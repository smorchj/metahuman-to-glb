"""
UE 5.7 WORK-IN-PROGRESS COPY. The 5.6.1 probe walks a `BP_<Name>` Blueprint
for component meshes; 5.7 collapsed that into a single `MetaHumanCharacter`
asset and this walker will find nothing. Rewrite to:
  1. Locate the `MetaHumanCharacter` asset for `<id>` under /Game/MetaHumans
     (exact path TBD — probe this as the very first thing).
  2. Enumerate its member SkeletalMesh references via the
     `MetaHumanCharacter` plugin's Python API.
  3. Dump the results into ../references/asset_paths.md so the export
     rewrite has a canonical reference.

Original 5.6.1 docstring follows:
